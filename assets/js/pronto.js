  ...
  accordionGroup.forEach(group => {
    ...
    const updatePanelScroll = () => {
      ...
      accordions.forEach(acc => {
        ...
        const panel = document.querySelector(`#section-${acc.dataset.panelTrigger}`)

        // scroll currently active associated panel into view
        window.requestAnimationFrame(() => {
          if (acc.open) panel.scrollIntoView({ behavior: 'smooth', block: 'start' })
        })
...
const buildHeightObserver = (el, slug, variableName) => {
  const observer = new ResizeObserver(([entry]) => {
    window.requestAnimationFrame(() => {
      document.documentElement.style.setProperty(variableName, entry.target.offsetHeight + 'px')
      Object.assign(window, { observers: { ...window.observers, slug: { height: entry.target.offsetHeight } } })
    })
  })
  el && observer.observe(el)
}

buildHeightObserver(document.querySelector('.prontoHeader'), 'header', '--header-height')
buildHeightObserver(document.querySelector('.prontoFooter'), 'footer', '--footer-height')
